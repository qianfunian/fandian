var igallery = null;
var igallery_iguid = '';
var igallery_last_iguid = 'fandian';
var igallery_index = 0;
var igallery_urls = [];
...
function Tuan_ItemGalleryPrev()
{
	if (igallery_index>0 && igallery_urls.length>0) {
		Fandian_AjaxLoading();
		document.getElementById('ig_container').innerHTML = '';
		
		igallery_index--;
		img = new Image();
		img.src = igallery_urls[igallery_index];
		img.onload = function(){
			Fandian_AjaxCompleted();
		};
		img.onerror = function(){
			this.src = FANDIAN_STATIC_URL + 'design/product/nopic.jpg';
			Fandian_AjaxCompleted();
		};
		
		document.getElementById('ig_container').appendChild(img);
	}
}

function Tuan_ItemGalleryNext()
{
	if (igallery_index>=0 && igallery_index<igallery_urls.length-1) {
		Fandian_AjaxLoading();
		document.getElementById('ig_container').innerHTML = '';
...
function PrepareReduceItem(VendorGuid, ItemGuid, count)
{
	...
	Fandian_SetCookie('tuan_items', arr.join(','));
	...
	if (Fandian_IsGuid(ThisItem.ItemGuid) && ThisItem) {
		...
		if (LAST_ITEM_COUNT>ThisItem.MinOrderQty || (LAST_ITEM_COUNT<=ThisItem.MinOrderQty && confirm('确定要删除这道菜吗？'))) {
			...
			if (LAST_ITEM_COUNT==ThisItem.MinOrderQty) {
				...
				lidom = document.getElementById('titem_'+ThisItem.ItemGuid);
				lidom.className = lidom.className.toString().replace('selected_pic', '');
...
function AddOrderItemRow(VendorGuid, ItemGuid)
{
	...
	if (ThisItem!=null) {
		wrapper = document.getElementById('order_vendor_items_'+VendorGuid);
...
{
	if (confirm('确定吗？')) {
		Fandian_SetCookie('tuan_items', '');	
		Fandian_SetCookie('remarks', '');
		
		dom = document.getElementById('order_vendors');
		
		for (i in PREPARED_ITEMS) {
			lidom = document.getElementById('titem_'+PREPARED_ITEMS[i]);
			licss = lidom.className;
			lidom.className = licss.toString().replace('selected_pic', '');
		}
...
function InitFirstItem(VendorGuid)
{
	...
	wrapper = document.getElementById('order_vendors');
	...
	html += "<li class='order_vendor_item_row_4'>&nbsp;</li>";
	html += "</ul>";
	html += "</li>";
	html += "</ol>";
	html += "<ol class='order_vendor_items' id='order_summary_"+_this_vendor_guid+"'>";
	html += "<li>";
	html += "<span style='float:left;'>外送费：￥<span id='order_freight_"+_this_vendor_guid+"'>"+_this_vendor_freight+"</span></span>";
	html += "<span style='float:left; margin-left: 100px;'>打包盒：<span id='order_boxes_"+_this_vendor_guid+"'>0</span>，共：￥<span id='order_boxes_price_"+_this_vendor_guid+"'>0</span></span>";
	html += "</li>";
...
function Tuan_AddItem(VendorGuid, ItemGuid)
{
	...
	if (Fandian_IsGuid(ItemGuid)) {
		...
		lidom = document.getElementById('titem_'+ItemGuid);
		licss = lidom.className;
		lidom.className = licss.toString().replace('selected_pic', '')+' selected_pic';
	}
}

function Tuan_ShowItemGallery(ItemGuid, ItemName)
{
	document.getElementById('igallery_title').innerHTML = ItemName;
	igallery_index = 0;
	igallery_iguid = ItemGuid;
	igallery.open();
}
...
/*	if (ORDER_IS_PRE==true && !CheckPreTime()) {
	...
	if (ErrorMsg!='') {
		...
		Fandian_WaitAjax(function(){
			Fandian_AjaxLoading();
			window.location = FANDIAN_BASE_URL+'order/confirm?from=tuan';
		});
	}
}
 ...
 * 
 */
function FixOrderWidget()
{
	y = Fandian_PageY();
	ow = document.getElementById('order_widget');
	on = document.getElementById('order_note');
	rm = document.getElementById('rmain');
	offset = 340;

	if (BROWSER.ie) {
		if (document.body.style.Height) {
			BH = parseInt(document.body.style.Height);
		} else {
			BH = parseInt(document.body.scrollHeight);
		}
		offset = 390;
	} else {
		BH = parseInt(document.documentElement.clientHeight+window.scrollMaxY);
	...
	MH = BH - offset - parseInt(ow.clientHeight);

	if (BROWSER.support_css_fixed) {
		if (y>256) {
			ow.style.position = 'fixed';
			ow.style.top = y>MH ? '-'+(y-MH)+'px' : '4px';
		...
		rm.style.width = '350px';
		
		if (y>256) {
			ow.style.position = 'absolute';
			ow.style.top = y>MH ? (MH)+'px' : (y)+'px';
			on.style.display = 'none';
		} else {
			ow.style.position = '';
			ow.style.top = '0px';
...
function Tuan_RenderItemGallery()
{
	if (igallery_last_iguid!=igallery_iguid) {
		var iajax = new Fandian_Ajax();
		iajax.get(FANDIAN_BASE_URL+'tuan/gallery?ItemGuid='+igallery_iguid, function(json){
			gs = json.urls.length;
			igallery_urls = json.urls;
			document.getElementById('igallery_title').innerHTML +=   '(共'+gs+'张图片)';
...
function InitTypeYPos()
{
	var t_offset = 30;
	
	if (TYPE_Y_POS.length==0) {
		for(i=0;i<ITEM_TYPES;i++) {
			_k = i+1;
			_dom = document.getElementById('pr0'+_k);
			_ndom = document.getElementById('pr0'+(_k+1));
			
			_now = Fandian_DomPos(_dom);
			if (_ndom) {
				_next = Fandian_DomPos(_ndom);
			} else {
				_next = {x:0, y:99999};
			}
			
			TYPE_Y_POS.push([_now.y-t_offset, _next.y-t_offset])
		}
	}
}

function FixTypeBanner()
{
	var t_offset = 30;
	var per_height = 20;
	var scroll2type = '';
	var fixed_offset = 254;
	var ow = document.getElementById('vendor_float_banner');
	...
	if (BROWSER.support_css_fixed) {
		if (y>fixed_offset) {
			...
			for (i=0;i<TYPE_Y_POS.length;i++) {
				_from = TYPE_Y_POS[i][0];
				_to = TYPE_Y_POS[i][1];
				
				this_dom = document.getElementById('fb_'+(i+1));
				
				if (y>_from && y<=_to) {
					this_dom.style.position = 'fixed';
					this_dom.style.top = t_offset+'px';
				} else {
					this_dom.style.position = '';
				}
			}
		} else {
			ow.style.position = '';
			...
			for (i=0;i<TYPE_Y_POS.length;i++) {
				document.getElementById('fb_'+(i+1)).style.position = '';
			}
		}
	} else {
		if (y>fixed_offset) {
			...
			for (i=0;i<TYPE_Y_POS.length;i++) {
				...
				if (y>_from && y<=_to) {
					this_dom.style.position = 'absolute';
					this_dom.style.top = (y+t_offset)+'px';
				} else {
					this_dom.style.position = '';
				}
			}
...
document.ready(function(){
	...
	igallery = new Fandian_Modal('modal_item_gallery', {
		width: 500,
		height: 310,
		top: 150,
		afterOpen: Tuan_RenderItemGallery
	});
});