...
class Msd_Dao_Table_Web_Sysuser extends Msd_Dao_Table_Web
{
	...
	{	
		...
		$this->_name = $this->prefix.'SysUser';
	...
	public function &search(&$pager, array $where=array(), array $order=array())
	{
		...
		if ($where['Username']) {
			$select->where('Username LIKE ?', '%'.$where['Username'].'%');
			$cSelect->where('Username LIKE ?', '%'.$where['Username'].'%');
		}
