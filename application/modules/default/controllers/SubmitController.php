...
class SubmitController extends Msd_Controller_Default
{
	...
	public function indexAction()
	{
		...
		$gguid = trim($this->_request->getPost('giftcard'));
		...
		if ($result) {
			...
			if(!empty($gguid)){
				$gtTable = Msd_Dao::table ( 'giftticket' );
                                $row = $gtTable->verify ( md5 ( strtoupper ( $gguid ) ) );
				if($row){
					$oTable->giftccgc ( $phone, md5 ( strtoupper ( $gguid ) ) );
				}else{
					Msd_Log::getInstance()->debug('调用失败-礼品好不存在:'.$gguid);
				}
			}
			}catch(Exception $e){
